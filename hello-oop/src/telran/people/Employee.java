...
public class Employee extends Person {
	...
	public Employee(long id, int birthYear, String email, int basicSalary) {
		...
		if(birthYear < minBirthYear || birthYear > maxBirthYear) {
			throw new IllegalArgumentException(String.format("%d - wrong birth year, " + "should be in range [%d = %d}", birthYear,
	...
	public void setBasicSalary(int basicSalary) {
		...
		if(basicSalary < minBasicSalary) {
			throw new IllegalArgumentException(String.format("%d - wrong basic salary, " + "cannot be less than [%d]", basicSalary,
					minBasicSalary));
		}
